...
describe("cancelOption", function () {
    ...
    it("reverts with non-existent optionID", async function () {
        await expect(NFTOptCTR.connect(buyer).cancelOption(9999)).to.be.revertedWith("INVALID_OPTION_ID");
    });
    ...
    it("reverts when option state is not OPEN", async function () {
        ...
        await expect(NFTOptCTR.connect(buyer).cancelOption(1)).to.be.revertedWith("INVALID_OPTION_STATE");
    ...
    it("reverts when non-participant tries to cancel", async function () {
        ...
        // Try to cancel
        await expect(NFTOptCTR.connect(seller).cancelOption(1)).to.be.revertedWith("NOT_AUTHORIZED");

        // Fast-foward EVM by interval + 1 days
        await increaseEVMTimestampBy(dummyOptionRequest.interval / SECONDS_IN_A_DAY + 1);

        // Try to cancel again
        await expect(NFTOptCTR.connect(nonParticipant).cancelOption(1)).to.be.revertedWith("NOT_AUTHORIZED");

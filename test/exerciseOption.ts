...
describe("exerciseOption", function () {
    ...
    it("reverts when buyer isn't the owner of option NFT after option is in effect", async function () {
        ...
        // Exercise option
        await expect(NFTOptCTR.connect(buyer).exerciseOption(1)).to.be.revertedWith("NFT_NOT_OWNER");
    });

    it("reverts when option state is not OPEN", async function () {
        await expect(NFTOptCTR.connect(buyer).exerciseOption(3)).to.be.revertedWith("INVALID_OPTION_STATE");
    });

    it("fails when contract address is not approved to transfer NFT", async function () {
        await expect(NFTOptCTR.connect(buyer).exerciseOption(1)).to.be.revertedWith("NOT_APPROVED_TO_TRANSFER_NFT");
    });
    ...
    it("succeeds when contract address is approved to transfer NFT", async function () {
        ...
        // Exercise option
        await expect(NFTOptCTR.connect(buyer).exerciseOption(1)).to.emit(NFTOptCTR, "Exercised").withArgs(1);
    });
    ...
    it("fails when european option is exercised before the expiration day", async function () {
        ...
        // Exercise option
        await expect(NFTOptCTR.connect(buyer).exerciseOption(2)).to.be.revertedWith("EXERCISE_WINDOW_IS_CLOSED");
    });
    ...
    it("succeeds when european option is exercised on expiration day", async function () {
        ...
        // Exercise option
        await expect(NFTOptCTR.connect(buyer).exerciseOption(2)).to.emit(NFTOptCTR, "Exercised").withArgs(2);
    });
    ...
    it("succeeds when exercised and buyer's ETH balance has increased by strike price", async function () {
        ...
        // Exercise option
        await expect(tx).to.emit(NFTOptCTR, "Exercised").withArgs(1);
    ...
    it("succeeds when exercised and seller has ownership of NFT transfered ", async function () {
        ...
        // Exercise option
        await expect(NFTOptCTR.connect(buyer).exerciseOption(id)).to.emit(NFTOptCTR, "Exercised").withArgs(id);
